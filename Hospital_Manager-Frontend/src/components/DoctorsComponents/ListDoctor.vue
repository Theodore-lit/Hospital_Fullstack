...
const filteredDoctor = computed(() => {
  return doctorStore.doctors.filter((t) =>
...
function editDoctor(doctor) {
  doctorStore.setEditDoctor(doctor)
  doctorStore.openForm()
  console.log(doctorStore.editedDoctor)
}
...
function detailsDoctor(doctor) {
  doctorStore.setDetailsDoctor(doctor)
  router.push({ name: 'details-admin' })
}
</script>

<template>
  <form class="w-full max-w-xl mx-auto mt-10 mb-12">
    <div class="relative flex items-center group">
      <input
        v-model="search"
        type="text"
        placeholder="Search tools, doctors, patients..."
        class="w-full h-14 pl-18 pr-32 rounded-2xl bg-white/80 backdrop-blur-md border border-gray-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-100 transition-all duration-300 text-gray-700 placeholder-gray-400"
      />
  ...
  <div class="mx-14 mt-10">
    ...
    >
      <div>
        <h2 class="text-2xl font-bold text-gray-900">Annuaire des Docteurs</h2>
        <p class="text-sm text-gray-500">Gérez les praticiens enregistrés et leurs informations</p>
      </div>
      ...
      >
        ...
        <span class="relative">Ajouter un Docteur</span>
      </button>
    </div>
          ...
          >
            <td class="p-5 whitespace-nowrap">
              <div class="flex items-center gap-3">
                ...
                <div class="text-sm" @click="detailsDoctor(doctor)">
                  <p class="font-bold text-gray-900">{{ doctor?.name }}</p>
                  <p class="text-gray-500 text-xs">{{ doctor.email }}</p>
                </div>
              </div>
            </td>
          ...
          <tr v-if="doctorStore.doctors.length == 0">
            <td colspan="5" class="p-16 text-center">
              <div class="flex flex-col items-center justify-center">
                ...
                <p class="text-gray-400 italic text-lg">Aucun docteur enregistré pour le moment.</p>
              </div>
            </td>
          </tr>
        </tbody>
